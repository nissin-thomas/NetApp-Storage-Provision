...
i = 0
while i < total_luns:
    con.run('vol create -volume ' + lun_name[i] + '_VOL -vserver SVM_SAN_01_EHS -aggregate ' + aggr_name[i] + ' -size ' + str(round(vol_size[i])) + 'g -state online -policy default -unix-permissions ---rwxr-xr-x -type RW -snapshot-policy none -foreground true -security-style unix -percent-snapshot-space 0 -space-guarantee none -autosize-mode grow_shrink -autosize-grow-threshold-percent 95 -autosize-shrink-threshold-percent 50')
